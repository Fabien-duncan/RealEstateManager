...
class FilterViewModel:ViewModel() {
    ...
    fun onMinPhotosChange(number: String?) {
        state = state.copy(minPhotos = NumberUtils.convertToIntOrNull(number))
    }
    fun onMinCreatedDateChanged(date: Date):Boolean{
        state = state.copy(minCreatedDate = date)
        return checkCreatedDatesAreValid()
    }
    fun onMaxCreatedDateChanged(date: Date):Boolean{
        state = state.copy(maxCreatedDate = date)
        return checkCreatedDatesAreValid()
    }
    ...
    private fun checkCreatedDatesAreValid():Boolean{
        return if (state.minCreatedDate != null && state.maxCreatedDate != null) state.minCreatedDate!!.before(state.maxCreatedDate) else true
    }
...
data class FilterState(
    ...
    val minCreatedDate:Date?=null,
    val maxCreatedDate:Date?=null,
